...
def Open_program():
    ...
    while Open_program_on == "true":
        ...
        speech_for_Open_program = r.recognize_google(
            audio_for_Open_program, language="ru_RU")
        print("Вы сказали " + speech_for_Open_program)

    if speech_for_Open_program.find("браузер") >= 0:
        os.system("C:\Program Files\Google\Chrome\Application\chrome.exe")
        voice.say("Браузер открыт")
        Open_program_on = "off"

    # elif speech_for_Open_program.find("") >= 0:
    #    os.startfile("")
    #    voice.say("")
    #    Open_program_on = "off"
#
